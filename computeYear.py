...
def readData():
    ...
    for line in d:
        ...
        litag.append(line[6])
        litime.append(line[1][:4])
        pass
    return (lix,liy,litag,litime)
...
for i in range(len(x)):
    dataList.append([x[i],y[i],tags[i],times[i]])
...
for item in dataList:
    ...
    if item[2]=='Positive ID' and item[3]=='2020':
...
for item in positive:
    cache.append([beeMap[item[0],item[1]],item])
    if beeMap[item[0]][item[1]] != -1:
        if beeMap[item[0]][item[1]] <= K:
